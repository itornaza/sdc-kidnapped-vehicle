      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (!pf.initialized()) {
            ...
            // Predict the vehicle's next state from previous (noiseless control)
            // data
          ...
          pf.updateWeights(sensor_range, sigma_landmark,
                           noisy_observations, map);
          pf.resample();
